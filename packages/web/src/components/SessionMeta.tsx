...
import { Cpu, Box, GitBranch, History, X, Loader2 } from "lucide-react";
import {
  fetchEnvironment, fetchSessionHistory,
  EnvironmentResponse, SessionHistoryRecord, AuthError,
} from "../lib/api";
...
export function SessionMeta({ sessionToken, onAuthFailed }: Props) {
  ...
  async function openHistory() {
    ...
    setLoading(true);
    try {
      const r = await fetchSessionHistory(sessionToken, 50);
      ...
      >
        <header className="flex items-center justify-between px-4 py-3 border-b border-line">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-mono text-xs uppercase tracking-wider text-muted">Session history</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-auto">
          {loading && (
            <div className="p-6 flex items-center gap-2 text-muted text-sm">
              <Loader2 className="w-4 h-4 animate-spin" /> Reading audit log…
            </div>
          )}
